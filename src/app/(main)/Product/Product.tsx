...
import { useProduct } from "@/src/hooks/product.hooks";
import {
...
export default function ProductScreen() {
	...
	const { data: getProduct, isLoading } = useProduct(id);
			...
			<ScrollView contentContainerStyle={{ paddingTop: 0, paddingBottom: 5 }}>
				...
				<ScrollView horizontal showsHorizontalScrollIndicator={false}>
					<View className="flex-row flex-wrap gap-2 px-4">
						{getProduct.data.images.map((img: any, index: number) => (
							<TouchableOpacity
								key={index}
								...
								<Image
									source={{ uri: img.path }}
									className="w-[75px] h-28 rounded-md"
								/>
							</TouchableOpacity>
						))}
					</View>
				...
				<View className=" px-4 mt-2 ">
					<Text
						...
						{getProduct?.data?.title?.shortTitle}
					</Text>
					<Text
						...
						{getProduct.data.title.longTitle}
					</Text>
				</View>
				<View className="flex-row items-center gap-3 px-4  ">
					<Text
						...
						{getProduct.data.price.discount}
					</Text>
					<Text
						className="text-gray-800 text-[16px]"
						style={{
				...
				<View className="flex-row pb-5 border-b border-gray-400 items-center justify-between  px-4  gap-2 pt-2 ">
					...
					<View className="flex-row items-center gap-2">
						{Array.from({ length: 5 }).map((_, index) => {
							if (index + 1 <= Math.floor(getProduct.data.rating || 0)) {
								return (
									<FontAwesome
										key={index}
										name="star"
							...
							} else if (index < (getProduct.data.rating || 0)) {
								return (
									<FontAwesome
										key={index}
					...
					<Text
						...
						{getProduct.data.reviews || 0} reviews
					</Text>
				</View>
